...
enum {
	BONUS_A, BONUS_B, BONUS_C, BONUS_D, BONUS_E, BONUS_F, BONUS_G, BONUS_NOTHING,
};
	...
	Mappy *updateStatus() {
		...
		if (fDIPSwitchChanged) {
			...
			case BONUS_B:
				...
				break;
			case BONUS_C:
				in[5] &= ~8, in[9] = in[9] & ~3 | 1;
				break;
			case BONUS_D:
				in[5] |= 8, in[9] = in[9] & ~3 | 1;
				break;
			case BONUS_E:
				in[5] &= ~8, in[9] = in[9] & ~3 | 2;
				break;
			case BONUS_F:
				in[5] |= 8, in[9] = in[9] & ~3 | 2;
				break;
...
//				in[7] &= ~4;
	...
	Mappy *updateInput() {
		...
		if (port[8] == 1)
			...
			memcpy(port + 1, vector<uint8_t>{8, 4, 6, 0xe, 0xd, 9, 0xd}.data(), 7);
		if (port[0x18] == 4)
			memcpy(port + 0x10, vector<uint8_t>{in[5], in[9], in[6], in[6], in[7], in[7], in[8], in[8]}.data(), 8);
		else if (port[0x18] == 5)
			memcpy(port + 0x11, vector<uint8_t>{8, 4, 6, 0xe, 0xd, 9, 0xd}.data(), 7);
		return edge = in[3] ^ 0xf, this;
	}
