...
dist/baraduke.wasm: $(addprefix src/,baraduke.cpp cpu.cpp baraduke.h mc6809.h mc6801.h cpu.h c30.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/chackn_pop.wasm: $(addprefix src/,chackn_pop.cpp z80.cpp cpu.cpp chackn_pop.h z80.h mc6805.h cpu.h ay-3-8910.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/crush_roller.wasm: $(addprefix src/,crush_roller.cpp z80.cpp cpu.cpp crush_roller.h z80.h cpu.h pac-man_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/digdug.wasm: $(addprefix src/,digdug.cpp z80.cpp cpu.cpp digdug.h z80.h mb8840.h cpu.h pac-man_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/digdug_ii.wasm: $(addprefix src/,digdug_ii.cpp cpu.cpp digdug_ii.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/dragon_buster.wasm: $(addprefix src/,dragon_buster.cpp cpu.cpp dragon_buster.h mc6809.h mc6801.h cpu.h c30.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)
...
dist/grobda.wasm: $(addprefix src/,grobda.cpp cpu.cpp grobda.h mc6809.h cpu.h mappy_sound.h sound_effect.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/libble_rabble.wasm: $(addprefix src/,libble_rabble.cpp cpu.cpp libble_rabble.h mc6809.h mc68000.h cpu.h mappy_sound.h utils.h)
	emcc -O3 -s INITIAL_MEMORY=33554432 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/mappy.wasm: $(addprefix src/,mappy.cpp cpu.cpp mappy.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/metro-cross.wasm: $(addprefix src/,metro-cross.cpp cpu.cpp metro-cross.h mc6809.h mc6801.h cpu.h c30.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/motos.wasm: $(addprefix src/,motos.cpp cpu.cpp motos.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/pac-land.wasm: $(addprefix src/,pac-land.cpp cpu.cpp pac-land.h mc6809.h mc6801.h cpu.h c30.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/pac-man.wasm: $(addprefix src/,pac-man.cpp z80.cpp cpu.cpp pac-man.h z80.h cpu.h pac-man_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/pac_and_pal.wasm: $(addprefix src/,pac_and_pal.cpp cpu.cpp pac_and_pal.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/pengo.wasm: $(addprefix src/,pengo.cpp z80.cpp cpu.cpp pengo.h z80.h cpu.h pac-man_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/phozon.wasm: $(addprefix src/,phozon.cpp cpu.cpp phozon.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)
...
dist/sky_kid.wasm: $(addprefix src/,sky_kid.cpp cpu.cpp sky_kid.h mc6809.h mc6801.h cpu.h c30.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)
...
dist/super_pac-man.wasm: $(addprefix src/,super_pac-man.cpp cpu.cpp super_pac-man.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)

dist/the_tower_of_druaga.wasm: $(addprefix src/,the_tower_of_druaga.cpp cpu.cpp the_tower_of_druaga.h mc6809.h cpu.h mappy_sound.h utils.h)
	emcc -O3 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)
...
dist/toypop.wasm: $(addprefix src/,toypop.cpp cpu.cpp toypop.h mc6809.h mc68000.h cpu.h mappy_sound.h utils.h)
	emcc -O3 -s INITIAL_MEMORY=33554432 --no-entry -Wno-shift-op-parentheses -o $@ $(filter %.cpp,$^)
